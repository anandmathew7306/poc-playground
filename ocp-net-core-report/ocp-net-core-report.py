#!/usr/bin/env python3
"""
ocp-net-core-report.py — generate a core-networking reference report for an
OpenShift cluster.

Covers host/node networking only: cluster overview, nodes, NICs, bonds,
OVS bridges, VLANs, VRFs & routing. MetalLB / load-balancer detail is covered
by the sibling ocp-net-metallb-report script; ocp-net-report combines both.

Read-only. Runs `oc get ... -o json` and prints markdown tables.
Requires: python3 + oc (logged in). No pip packages.

Usage:
    python3 ocp-net-core-report.py                 # print to stdout
    python3 ocp-net-core-report.py --all-nics      # include down/unused NICs and geneve device
    python3 ocp-net-core-report.py > report.md     # save to file
"""
...
def main():
    ...
    p = argparse.ArgumentParser(
        description="Generate a markdown core-networking reference report for an "
                    "OpenShift cluster (read-only).")
    p.add_argument("--all-nics", action="store_true",
                   help="include down/unused NICs and the geneve device")
    args = p.parse_args()
    SHOW_ALL_NICS = args.all_nics

    print("# OpenShift Network Reference — Core Networking\n")
