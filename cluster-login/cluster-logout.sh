#!/usr/bin/env bash
# Drop every local kubeconfig session and unset the current context.
# Local only — does not change any cluster.
set -euo pipefail
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: cluster-logout

Remove all local cluster sessions and clear the current context.
After this, oc/kubectl have no active cluster until you cluster-login again.
EOF
    exit 0
fi
...
def print_box(lines):
    ...
    for line in clipped:
        print(dim_box("│ ") + paint(PINK, line) + dim_box(" │"))
    print(dim_box("└" + "─" * between + "┘"))
